class BalanceChecksController < ApplicationController
  ...
  def check_balance
    ...
    if @account.present? and @account.account_balances.count >= @account_balance_num + 1
      ...
      @transactions = current_user.transactions.where(transaction_conditions).page(params[:transactions_page]).per_page(10)
      @total_spending = current_user.transactions.where(transaction_conditions).sum(:amount)
      @transfers_in = @account.incoming_transfers.where(transfer_in_conditions).page(params[:transfers_in_page]).per_page(10)
      @transfers_out = @account.outgoing_transfers.where(transfer_out_conditions).page(params[:transfers_out_page]).per_page(10)
